...
test1 <- test %>% filter(commiter == 'Preethi Narayanan' )

test2 <- test %>% filter(commiter == "Kristine Dinh")

test3 <- test %>% filter(commiter == "Jason Perone")

test4 <- test %>% filter(commiter == "Eric Lessert")

test5 <- test %>% filter(commiter == "Matthew Johns")
...
plot(test1$commit_date, test1$commit_no, col = 'green',type = 'l')
